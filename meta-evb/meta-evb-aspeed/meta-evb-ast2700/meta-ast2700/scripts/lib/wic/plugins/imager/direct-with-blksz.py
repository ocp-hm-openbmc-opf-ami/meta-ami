...
class DirectWithBlockSizePlugin(ImagerPlugin):
    """
    ...
    def __init__(self, wks_file, rootfs_dir, bootimg_dir, kernel_dir,
        ...
        image_path = self._full_path(self.workdir, self.parts[0].disk, "direct-with-blksz")
    ...
    def print_info(self):
        ...
        extension = "direct-with-blksz" + {"gzip": ".gz",
                                "bzip2": ".bz2",
                                "xz": ".xz",
...
class PartitionedImage():
    ...
    def __init__(self, path, ptable_format, partitions, native_sysroot=None, extra_space=0):
        ...
        self.sector_size = int(get_bitbake_var('WIC_SECTOR_SIZE'))
    ...
    def create(self):
        ...
        exec_native_cmd("export PARTED_SECTOR_SIZE=%d; parted -s %s mklabel %s" %
                        (self.sector_size, self.path, self.ptable_format), self.native_sysroot)
        ...
        for part in self.partitions:
            ...
            if part.part_name:
                ...
                exec_native_cmd("export BLK_DEFAULT_SECTOR_SIZE=%d; sfdisk --part-label %s %d %s" % \
                                         (self.sector_size, self.path, part.num,
                                          part.part_name), self.native_sysroot)

            if part.part_type:
                logger.debug("partition %d: set type UID to %s",
                             part.num, part.part_type)
                exec_native_cmd("export BLK_DEFAULT_SECTOR_SIZE=%d; sfdisk --part-type %s %d %s" % \
                                         (self.sector_size, self.path, part.num,
            ...
            if part.uuid and self.ptable_format == "gpt":
                logger.debug("partition %d: set UUID to %s",
                             part.num, part.uuid)
                exec_native_cmd("export BLK_DEFAULT_SECTOR_SIZE=%d; sfdisk --part-uuid %s %d %s" % \
            ...
            if part.label and self.ptable_format == "gpt":
                logger.debug("partition %d: set name to %s",
                             part.num, part.label)
                exec_native_cmd("export PARTED_SECTOR_SIZE=%d; parted -s %s name %d %s" % \
                                (self.sector_size, self.path, part.num, part.label),
                                self.native_sysroot)
