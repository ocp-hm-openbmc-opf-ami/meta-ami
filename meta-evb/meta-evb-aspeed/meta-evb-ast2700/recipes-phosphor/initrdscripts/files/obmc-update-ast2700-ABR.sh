...
findmtd() {
	# AST2700 ABR for single flash mode
	if [ "$1" = "u-boot" ] || [ "$1" = "u-boot-env" ] || [ "$1" = "kernel" ] || [ "$1" = "rofs" ] || [ "$1" = "rwfs" ]; then
		cmdline=$(cat /proc/cmdline)
		if [ "$(echo "$cmdline" | grep -c "rootfs=rofs-a")" -gt 0  ]; then
			fs="$1-a"
		elif [ "$(echo "$cmdline" | grep -c "rootfs=rofs-b")" -gt 0 ]; then
			fs="$1-b"
		else
			fs=$1
			m=$(grep -xl "$fs" /sys/class/mtd/*/name)
			# If use aspeed-flash-layout-256-abr.dtsi, but doesn't enable ABR.
			# Use u-boot-a, kernel-a, rofs-a, rwfs-a as default file system.
			if [ -z "$m" ]; then
				fs="$1-a"
			fi
		fi
