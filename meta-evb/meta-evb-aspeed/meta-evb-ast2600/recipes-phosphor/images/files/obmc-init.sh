...
#Run veritysetup create if, verity env available
if [ -f "dm-verity.env" ]; 
then
	source /dm-verity.env
	veritysetup create rofs --hash-offset $DATA_SIZE /dev/mtdblock4 /dev/mtdblock4 $ROOT_HASH
	rodev=/dev/mapper/rofs
fi
