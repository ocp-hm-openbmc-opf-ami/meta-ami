...
# Only activate this shadow when the feature is enabled
python __anonymous() {
    if bb.utils.contains('IMAGE_FEATURES', 'onetree-dual-image-common-conf', True, False, d) or \
       bb.utils.contains('DISTRO_FEATURES', 'onetree-dual-image-common-conf', True, False, d):
        d.appendVar('OVERRIDES', ':onetree-dual-image-common-conf')
        bb.note("image_types_phosphor (shadow): 'rwfs' removed (onetree-dual-image-common-conf active)")
}

# UBI: Only kernel + rofs, no rwfs
do_make_ubi:onetree-dual-image-common-conf () {
    cfg=ubinize-${IMAGE_NAME}.cfg
    rm -f $cfg ubi-img
...
do_make_ubi:onetree-dual-image-common-conf[dirs] = "${S}/ubi"
do_make_ubi:onetree-dual-image-common-conf[depends] += " \
    ${PN}:do_image_${@d.getVar('FLASH_UBI_BASETYPE').replace('-', '_')} \
    virtual/kernel:do_deploy \
    u-boot:do_deploy \
    mtd-utils-native:do_populate_sysroot \
"

# Static NOR: Only up to rofs, no rwfs appended
python do_generate_static:onetree-dual-image-common-conf () {
    import os, subprocess

    bb.build.exec_func("do_mk_static_nor_image", d)
    nor_image = os.path.join(d.getVar('IMGDEPLOYDIR'),
                             '%s.static.mtd' % d.getVar('IMAGE_NAME'))
    ...
    def _append_image(imgpath, start_kb, finish_kb):
        ...
        if imgsize > maxsize:
            bb.fatal("Image '%s' is %d bytes too large!" % (imgpath, imgsize - maxsize))
        subprocess.check_call(['dd', 'bs=1k', 'conv=notrunc',
                               'seek=%d' % start_kb, 'if=%s' % imgpath, 'of=%s' % nor_image])

    uboot_offset = int(d.getVar('FLASH_UBOOT_OFFSET'))
    spl_binary = d.getVar('SPL_BINARY')
    if spl_binary:
        _append_image(os.path.join(d.getVar('DEPLOY_DIR_IMAGE'),
                                   'u-boot-spl.%s' % d.getVar('UBOOT_SUFFIX')),
                      int(d.getVar('FLASH_UBOOT_OFFSET')),
                      int(d.getVar('FLASH_UBOOT_SPL_SIZE')))
        uboot_offset += int(d.getVar('FLASH_UBOOT_SPL_SIZE'))

    _append_image(os.path.join(d.getVar('DEPLOY_DIR_IMAGE'),
                               'u-boot.%s' % d.getVar('UBOOT_SUFFIX')),
                  uboot_offset,
                  int(d.getVar('FLASH_UBOOT_ENV_OFFSET')))

    _append_image(os.path.join(d.getVar('DEPLOY_DIR_IMAGE'),
                               d.getVar('FLASH_KERNEL_IMAGE')),
                  int(d.getVar('FLASH_KERNEL_OFFSET')),
                  int(d.getVar('FLASH_ROFS_OFFSET')))

    _append_image(os.path.join(d.getVar('IMGDEPLOYDIR'),
                               '%s.%s' % (d.getVar('IMAGE_LINK_NAME'),
                                          d.getVar('IMAGE_BASETYPE'))),
                  int(d.getVar('FLASH_ROFS_OFFSET')),
                  int(d.getVar('FLASH_SIZE')))

    bb.build.exec_func("do_mk_static_symlinks", d)
}

do_mk_static_symlinks:onetree-dual-image-common-conf () {
    cd ${IMGDEPLOYDIR}
    ln -sf ${IMAGE_NAME}.static.mtd ${IMGDEPLOYDIR}/${IMAGE_LINK_NAME}.static.mtd
    do_generate_image_uboot_file ${IMGDEPLOYDIR}/image-u-boot
    ln -sf ${IMAGE_NAME}.static.mtd ${IMGDEPLOYDIR}/flash-${MACHINE}
    ...
    ln -sf ${FLASH_KERNEL_IMAGE} ${IMGDEPLOYDIR}/image-kernel
    ln -sf ${IMAGE_LINK_NAME}.${IMAGE_BASETYPE} ${IMGDEPLOYDIR}/image-rofs
    # intentionally no image-rwfs
}

# Tar packaging: exclude image-rwfs
do_generate_static_tar:onetree-dual-image-common-conf () {
    ln -sf ${S}/MANIFEST MANIFEST
    ln -sf ${S}/publickey publickey

    do_generate_image_uboot_file image-u-boot
    ln -sf ${DEPLOY_DIR_IMAGE}/${FLASH_KERNEL_IMAGE} image-kernel
    ln -sf ${IMGDEPLOYDIR}/${IMAGE_LINK_NAME}.${IMAGE_BASETYPE} image-rofs

    make_signatures image-u-boot image-kernel image-rofs MANIFEST publickey

    tar -h -cvf ${IMGDEPLOYDIR}/${IMAGE_NAME}.static.mtd.tar \
        image-u-boot image-kernel image-rofs MANIFEST publickey ${signature_files}

    cd ${IMGDEPLOYDIR}
    ln -sf ${IMAGE_NAME}.static.mtd.tar ${IMGDEPLOYDIR}/${IMAGE_LINK_NAME}.static.mtd.tar
}
do_generate_static_tar:onetree-dual-image-common-conf[dirs] = " ${S}/static"
do_generate_static_tar:onetree-dual-image-common-conf[depends] += " \
    ${PN}:do_image_${@d.getVar('IMAGE_BASETYPE').replace('-', '_')} \
    virtual/kernel:do_deploy \
    u-boot:do_deploy \
    openssl-native:do_populate_sysroot \
    ${SIGNING_KEY_DEPENDS} \
...
do_generate_ubi_tar:onetree-dual-image-common-conf () {
    ln -sf ${S}/MANIFEST MANIFEST
    ln -sf ${S}/publickey publickey

    do_generate_image_uboot_file image-u-boot
    ln -sf ${DEPLOY_DIR_IMAGE}/${FLASH_KERNEL_IMAGE} image-kernel
    ln -sf ${IMGDEPLOYDIR}/${IMAGE_LINK_NAME}.${FLASH_UBI_BASETYPE} image-rofs

    make_signatures image-u-boot image-kernel image-rofs MANIFEST publickey

    tar -h -cvf ${IMGDEPLOYDIR}/${IMAGE_NAME}.ubi.mtd.tar \
        image-u-boot image-kernel image-rofs MANIFEST publickey ${signature_files}

    cd ${IMGDEPLOYDIR}
    ln -sf ${IMAGE_NAME}.ubi.mtd.tar ${IMGDEPLOYDIR}/${IMAGE_LINK_NAME}.ubi.mtd.tar
}
do_generate_ubi_tar:onetree-dual-image-common-conf[dirs] = " ${S}/ubi"
do_generate_ubi_tar:onetree-dual-image-common-conf[depends] += " \
    ${PN}:do_image_${@d.getVar('FLASH_UBI_BASETYPE').replace('-', '_')} \
    virtual/kernel:do_deploy \
    u-boot:do_deploy \
    openssl-native:do_populate_sysroot \
