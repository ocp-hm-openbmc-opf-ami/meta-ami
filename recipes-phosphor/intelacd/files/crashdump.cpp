...
static void logRunTime(cJSON *parent, timespec *start, char *key) {
  ...
  cd_snprintf_s(timeString, sizeof(timeString), "%.2fs", (double)timeVal / 1e9);
...
static bool savePeciWake(std::vector<CPUInfo> &cpuInfo) {
  ...
  for (CPUInfo &cpu : cpuInfo) {
    retval = peci_RdPkgConfig(cpu.clientAddr, vcuPeciWake, ON, sizeof(uint32_t),
...
static void loadInputFiles(std::vector<CPUInfo> &cpuInfo,
  ...
  for (CPUInfo &cpu : cpuInfo) {
    ...
    defaultStateSection = getCrashDataSection(inputFileInfo->buffers[cpu.model],
                                              "DefaultState", &enable);
    int defaultStateEnable = 1;
    if (defaultStateSection != NULL) {
      strcmp_s(defaultStateSection->valuestring, CRASHDUMP_VALUE_LEN, "Enable",
               &defaultStateEnable);
    } else {
    ...
    if (defaultStateEnable == ENABLE) {
      ...
      for (uint8_t i = 0; i < NUMBER_OF_SECTIONS; i++) {
        ...
        enable ? SET_BIT(cpu.sectionMask, i) : CLEAR_BIT(cpu.sectionMask, i);
      }
    } else {
...
static bool getCoreMasks(std::vector<CPUInfo> &cpuInfo, cpuidState cpuState) {
  ...
  for (CPUInfo &cpu : cpuInfo) {
    ...
    case cd_icxd:
      ...
      retval =
          peci_RdPCIConfigLocal(cpu.clientAddr, 14, 30, 3, 0xD0,
                                sizeof(coreMask0), (uint8_t *)&coreMask0, &cc);
      ...
      retval =
          peci_RdPCIConfigLocal(cpu.clientAddr, 14, 30, 3, 0xD4,
                                sizeof(coreMask1), (uint8_t *)&coreMask1, &cc);
      cpu.coreMaskRead.coreMaskCc = cc;
      cpu.coreMaskRead.coreMaskRet = retval;
      if (retval != PECI_CC_SUCCESS) {
        CRASHDUMP_PRINT(ERR, stderr,
    ...
    case cd_spr:
      ...
      retval = peci_RdEndPointConfigPciLocal(cpu.clientAddr, 0, 31, 30, 6, 0x80,
                                             sizeof(coreMask0),
      ...
      retval = peci_RdEndPointConfigPciLocal(cpu.clientAddr, 0, 31, 30, 6, 0x84,
                                             sizeof(coreMask1),
...
static bool getCHACounts(std::vector<CPUInfo> &cpuInfo, cpuidState cpuState) {
  ...
  for (CPUInfo &cpu : cpuInfo) {
    ...
    case cd_icxd:
      ...
      retval =
          peci_RdPCIConfigLocal(cpu.clientAddr, 14, 30, 3, 0x9C,
                                sizeof(chaMask0), (uint8_t *)&chaMask0, &cc);
      ...
      retval =
          peci_RdPCIConfigLocal(cpu.clientAddr, 14, 30, 3, 0xA0,
                                sizeof(chaMask1), (uint8_t *)&chaMask1, &cc);
      cpu.chaCountRead.chaCountCc = cc;
      cpu.chaCountRead.chaCountRet = retval;
      if (retval != PECI_CC_SUCCESS) {
        CRASHDUMP_PRINT(ERR, stderr,
    ...
    case cd_spr:
      ...
      retval = peci_RdEndPointConfigPciLocal(cpu.clientAddr, 0, 31, 30, 3, 0x9c,
                                             sizeof(chaMask0),
                                             (uint8_t *)&chaMask0, &cc);
      cpu.chaCountRead.chaCountCc = cc;
      cpu.chaCountRead.chaCountRet = retval;
      ...
      retval = peci_RdEndPointConfigPciLocal(cpu.clientAddr, 0, 31, 30, 3, 0xa0,
                                             sizeof(chaMask1),
                                             (uint8_t *)&chaMask1, &cc);
      cpu.chaCountRead.chaCountCc = cc;
      cpu.chaCountRead.chaCountRet = retval;
    ...
    cpu.chaCount = __builtin_popcount(chaMask0) + __builtin_popcount(chaMask1);
...
static void getCPUID(CPUInfo &cpuInfo) {
  ...
  if (retval != PECI_CC_SUCCESS) {
    CRASHDUMP_PRINT(ERR, stderr, "Cannot get CPUID! ret: (0x%x), cc: (0x%x)\n",
                    retval, cc);
  }
}
...
static cJSON *addSectionLog(
  ...
  if (logSectionJson->child == NULL) {
    ...
    if (ret != 0) {
      cJSON_AddStringToObject(logSectionJson, crashdump::dbgStatusItemName,
...
void fillSection(
  ...
  if (CHECK_BIT(cpuInfo->sectionMask, sectionName.section)) {
    ...
    logSection = addSectionLog(cpu, cpuInfo, sectionName.name, sectionLogFunc);
...
void fillSectionWithChild(
  ...
  if (CHECK_BIT(cpuInfo->sectionMask, sectionName.section)) {
    ...
    logSection = addSectionLog(cpu, cpuInfo, sectionName.name, sectionLogFunc2);
    if (logSection) {
      logCrashdumpVersion(logSection, cpuInfo, sectionName.record_type);
      logRunTime(logSection, sectionStart, timeStr);
    }
...
void fillMetaDataCPU(cJSON *crashlogData, CPUInfo cpuInfo,
  ...
  if (CHECK_BIT(cpuInfo.sectionMask, METADATA)) {
    ...
    logSection = addSectionLog(crashlogData, &cpuInfo, "METADATA", logSysInfo);
...
void createCrashdump(std::vector<CPUInfo> &cpuInfo,
  ...
  for (size_t i = 0; i < cpuInfo.size(); i++) {
    ...
    cJSON_AddItemToObject(processors, cpuString, cpu = cJSON_CreateObject());
    ...
    if (cpuInfo[i].cpuidRead.cpuidValid) {
      ...
      addDelayToSection(cpu, &cpuInfo[i], sectionNames[Section::BIG_CORE].name,
                        &crashdumpStart);
      ...
      fillSectionWithChild(cpu, &cpuInfo[i], sectionNames[Section::MCA],
                           logCoreMca, logUncoreMca, &sectionStart, timeStr);
  ...
  for (size_t i = 0; i < cpuInfo.size(); i++) {
    ...
    oemLogSection = addSectionLog(oemCpu, &cpuInfo[i], "oemdata", logOemData);
    if (oemLogSection) {
...
static void newOnDemandLog(std::vector<CPUInfo> &cpuInfo,
  ...
  createCrashdump(cpuInfo, onDemandLogContents, triggerTypeOnDemand, timestamp,
                  false);
}
...
static void newTelemetryLog(std::vector<CPUInfo> &cpuInfo,
  ...
  createCrashdump(cpuInfo, telemetryLogContents, triggerTypeOnDemand, timestamp,
                  true);
}
...
static void incrementCrashdumpCount() {
  ...
  conn->async_method_call(
      [](boost::system::error_code ec, const std::variant<uint8_t> &property) {
        if (ec) {
          CRASHDUMP_PRINT(ERR, stderr, "Failed to get Crashdump count\n");
          return;
        }
        const uint8_t *crashdumpCountVariant = std::get_if<uint8_t>(&property);
        if (crashdumpCountVariant == nullptr) {
          CRASHDUMP_PRINT(ERR, stderr, "Unable to read Crashdump count\n");
          return;
        }
        uint8_t crashdumpCount = *crashdumpCountVariant;
        if (crashdumpCount == std::numeric_limits<uint8_t>::max()) {
          CRASHDUMP_PRINT(ERR, stderr, "Maximum crashdump count reached\n");
          return;
        }
        // Increment the count
        crashdumpCount++;
        conn->async_method_call(
            [](boost::system::error_code ec) {
              if (ec) {
                CRASHDUMP_PRINT(ERR, stderr, "Failed to set Crashdump count\n");
              }
            },
            ...
            std::variant<uint8_t>{crashdumpCount});
      },
      "xyz.openbmc_project.Settings",
      "/xyz/openbmc_project/control/processor_error_config",
...
static void dbusAddStoredLog(const std::string &storedLogContents,
  ...
  if (numLogFiles > 0) {
    ...
    sscanf_s(namelist[numLogFiles - 1]->d_name, crashdumpFile, &crashdump_num);
    crashdump_num++;
  }
  ...
  for (int i = 0; i < numLogFiles; i++) {
    ...
    if ((i != 0) && (i <= (numLogFiles - (numStoredLogs - 1)))) {
      std::error_code ec;
      if (!(std::filesystem::remove(crashdumpDir / namelist[i]->d_name, ec))) {
        CRASHDUMP_PRINT(ERR, stderr, "failed to remove %s: %s\n",
...
int main() {
  ...
  ifaceStored->register_method(
      ...
      [&future, ifaceStored](const std::string &triggerType) {
        if (!crashdump::isPECIAvailable()) {
          // TODO: throw crashdump::PowerOffException();
        }
        if (future.valid() && future.wait_for(std::chrono::seconds(0)) !=
                                  std::future_status::ready) {
          // TODO: throw crashdump::LogInProgressException();
        }
  ...
  ifaceOnDemand->register_method("GenerateOnDemandLog", [&future]() {
    ...
    future = std::async(std::launch::async, []() {
      ...
      crashdump::newOnDemandLog(crashdump::cpuInfo, onDemandLogContents,
                                onDemandTimestamp);
      boost::asio::post(
          crashdump::io, [onDemandLogContents = std::move(onDemandLogContents),
                          onDemandTimestamp = std::move(onDemandTimestamp),
                          filename]() mutable {
            crashdump::dbusAddLog(onDemandLogContents, onDemandTimestamp,
  ...
  std::shared_ptr<sdbusplus::asio::dbus_interface> ifaceTelemetry =
      crashdump::server->add_interface(crashdump::crashdumpPath,
                                       crashdump::crashdumpTelemetryInterface);
  ...
  ifaceTelemetry->register_method("GenerateTelemetryLog", [&future]() {
    ...
    future = std::async(std::launch::async, []() {
      ...
      std::string filename =
          crashdump::crashdumpTelemetryFileRoot + telemetryTimestamp + ".json";
  ...
  if (std::filesystem::exists(crashdump::crashdumpDir)) {
    ...
    for (auto &p :
      ...
      if (std::regex_match(file, match, search)) {
        // Log Interface
        std::filesystem::path path =
            std::filesystem::path(crashdump::crashdumpPath) / match.str(1);
        std::shared_ptr<sdbusplus::asio::dbus_interface> ifaceLog =
            crashdump::server->add_interface(path.c_str(),
  ...
  ifaceRawPeci->register_method(
      "SendRawPeci", [](const std::vector<std::vector<uint8_t>> &rawCmds) {
        ...
        for (auto const &rawCmd : rawCmds) {
          ...
          EPECIStatus rc = peci_raw(rawCmd[0], rawCmd[2], &rawCmd[3], rawCmd[1],
                                    resp.data(), resp.size());
