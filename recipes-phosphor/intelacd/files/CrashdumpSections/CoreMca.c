...
static void coreMcaJson(uint32_t u32CoreNum, uint32_t u32ThreadNum,
  ...
  if (!isValid) {
    cd_snprintf_s(jsonItemString, CORE_MCA_JSON_STRING_LEN, CORE_MCA_NA, cc);
  ...
  if (ret != PECI_CC_SUCCESS) {
    ...
    cd_snprintf_s(jsonItemString, CORE_MCA_JSON_STRING_LEN, CORE_MCA_DATA_CC_RC,
                  u64CoreMcaData, cc, ret);
  } else {
    // Add the MCA register data to the Core MCA JSON structure
    cd_snprintf_s(jsonItemString, CORE_MCA_JSON_STRING_LEN, CORE_MCA_UINT64_FMT,
                  u64CoreMcaData);
  }
...
int logCoreMcaICXSPR(CPUInfo *cpuInfo, cJSON *pJsonChild) {
  ...
  for (uint32_t u32CoreNum = 0; (cpuInfo->coreMask >> u32CoreNum) != 0;
    ...
    for (uint32_t u32ThreadNum = 0; u32ThreadNum < CORE_MCA_THREADS_PER_CORE;
         u32ThreadNum++) {
      ...
      cJSON_ArrayForEach(itRegs, regList) {
        ...
        coreMcaJson(u32CoreNum, u32ThreadNum, &reg, 0, pJsonChild, 0, ret,
                    !CORE_MCA_VALID);
      }
      ...
      cJSON_ArrayForEach(itRegs, regList) {
        ...
        u64CoreMcaData = 0;
        cc = 0;
        ret = peci_RdIAMSR(cpuInfo->clientAddr, (u32CoreNum * 2) + u32ThreadNum,
                           reg.addr, &u64CoreMcaData, &cc);
        ...
        coreMcaJson(u32CoreNum, u32ThreadNum, &reg, u64CoreMcaData, pJsonChild,
                    cc, ret, CORE_MCA_VALID);
      }
    }
  nextCore:;
  }
