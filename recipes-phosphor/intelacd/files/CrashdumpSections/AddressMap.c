...
static void addressMapJsonICX(const char *regName,
  ...
  if (sRegData->bInvalid) {
    cd_snprintf_s(jsonItemString, AM_JSON_STRING_LEN, AM_FIXED_DATA_CC_RC, cc,
                  ret);
...
int logAddressMapEntriesICX1(CPUInfo *cpuInfo, cJSON *pJsonChild) {
  ...
  cJSON *itRegs = NULL;
  cJSON *itParams = NULL;
  regList = getCrashDataSectionAddressMapRegList(cpuInfo->inputFile.bufferPtr);
  ...
  cJSON_ArrayForEach(itRegs, regList) {
    ...
    cJSON_ArrayForEach(itParams, itRegs) {
      ...
      case ADDRESS_MAP_OFFSET:
        addrMapRegs.u16Reg = strtoull(itParams->valuestring, NULL, 16);
        break;
    ...
    case AM_REG_QWORD:
      for (uint8_t u8Dword = 0; u8Dword < 2; u8Dword++) {
        ret = peci_RdEndPointConfigPciLocal_seq(
            ...
            sizeof(uint32_t), (uint8_t *)&sRegData.uValue.u32[u8Dword], peci_fd,
            &cc);
