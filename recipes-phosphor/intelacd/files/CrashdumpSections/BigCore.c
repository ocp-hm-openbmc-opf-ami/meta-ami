...
static void logSqData(cJSON *sqDump, cJSON *thread, uint32_t u32CrashSize,
  ...
  *u32DataIndex += ret;
  cd_snprintf_s(jsonItemName, CD_JSON_STRING_LEN, CD_JSON_SQ_ENTRY_NAME, index);
  if (u32NumReads < *u32DataIndex) {
    if (retval != PECI_CC_SUCCESS) {
      ...
      cd_snprintf_s(jsonErrorString, CD_JSON_STRING_LEN, CD_JSON_DATA_CC_RC, cc,
                    retval);
...
static uint32_t registerDump(CPUInfo *cpuInfo, uint32_t u32CoreNum,
  ...
  cd_snprintf_s(jsonItemName, CD_JSON_STRING_LEN, CD_JSON_CORE_NAME,
                u32CoreNum);
  if ((*ppcore = cJSON_GetObjectItemCaseSensitive(pJsonChild, jsonItemName)) ==
      NULL) {
  ...
  cJSON_ArrayForEach(itItem, regList) {
    ...
    int ret =
        getJsonDataString(pu8Crashdump, u32CrashSize, u32DataIndex, reg.size,
    ...
    if (u32NumReads < u32DataIndex) {
      if (retval != PECI_CC_SUCCESS) {
        ...
        cd_snprintf_s(jsonErrorString, CD_JSON_STRING_LEN, CD_JSON_DATA_CC_RC,
                      cc, retval);
        strcat_s(jsonItemString, CD_JSON_STRING_LEN, jsonErrorString);
        cJSON_AddStringToObject(*ppthread, reg.name, jsonItemString);
        return u32CrashSize;
...
static void crashdumpJsonICX(CPUInfo *cpuInfo, uint32_t u32CoreNum,
  ...
  if (CD_JSON_NUM_SQ_ENTRIES * 8 == sqDataSize) {
    ...
    cJSON_AddStringToObject(sqDump, "_error_unexpected_size", jsonItemString);
    // Add the SQ data
    for (uint32_t i = 0; i < sqDataSize; i++) {
...
static inline bool isStoreDecoded(CPUInfo *cpuInfo, uint32_t totalInputSize,
                                  ...
                                  uint32_t u32version) {
  return (isBigCoreRegVersionMatch(cpuInfo->inputFile.bufferPtr, u32version) &&
          totalInputSize == (uCrashdumpVerSize.field.regDumpSize));
}

static inline uint32_t
calculateCrashdumpSize(CPUInfo *cpuInfo, UCrashdumpVerSize uCrashdumpVerSize) {
...
int logCrashdumpICXSPR(CPUInfo *cpuInfo, cJSON *pJsonChild) {
  ...
  ret = peci_CrashDump_Discovery(cpuInfo->clientAddr, PECI_CRASHDUMP_ENABLED, 0,
                                 0, 0, sizeof(uint8_t), &u8CrashdumpDisabled,
                                 &cc);
  ...
  ret = peci_CrashDump_Discovery(cpuInfo->clientAddr, PECI_CRASHDUMP_NUM_AGENTS,
                                 0, 0, 0, sizeof(uint16_t),
                                 (uint8_t *)&u16CrashdumpNumAgents, &cc);
  ...
  for (uint32_t u32CoreNum = 0; (cpuInfo->coreMask >> u32CoreNum) != 0;
    ...
    for (uint32_t u32ThreadNum = 0; u32ThreadNum < u32Threads; u32ThreadNum++) {
      // Get the crashdump size for this thread from the first read
      UCrashdumpVerSize uCrashdumpVerSize;
      ret = peci_CrashDump_GetFrame(cpuInfo->clientAddr, PECI_CRASHDUMP_CORE,
      ...
      for (uint32_t i = 1; i < (u32CrashdumpSize / sizeof(uint64_t)); i++) {
        ret = peci_CrashDump_GetFrame(cpuInfo->clientAddr, PECI_CRASHDUMP_CORE,
                                      u32CoreNum, 0, sizeof(uint64_t),
                                      (uint8_t *)&pu64Crashdump[i], &cc);
        u32NumReads = i;
        if (PECI_CC_SKIP_CORE(cc)) {
          CRASHDUMP_PRINT(ERR, stderr, "Error (%d) during GetFrame (num:%d)\n",
                          ret, i);
          gotoNextCore = true;
          break;
        }
      ...
      if (isStoreDecoded(cpuInfo, totalInputSize, uCrashdumpVerSize,
        ...
        case cd_icx2:
        case cd_icxd:
          crashdumpJsonICX(cpuInfo, u32CoreNum, threadId, u32CrashdumpSize,
                           u32NumReads, (uint8_t *)pu64Crashdump, pJsonChild,
                           cc, ret, u32version);
          break;
        case cd_spr:
          crashdumpJsonSPR(cpuInfo, u32CoreNum, threadId, u32CrashdumpSize,
                           u32NumReads, (uint8_t *)pu64Crashdump, pJsonChild,
                           cc, ret, u32version);
          break;
        default:
