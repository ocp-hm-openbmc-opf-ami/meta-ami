...
static void powerManagementJsonICX1(const char *regName,
  ...
  if (sRegData->bInvalid) {
    cd_snprintf_s(jsonItemString, PM_JSON_STRING_LEN, PM_FIXED_DATA_CC_RC, cc,
                  ret);
...
static void powerManagementPllJsonICX1(const char *secName, const char *regName,
  ...
  if ((section = cJSON_GetObjectItemCaseSensitive(pJsonChild, secName)) ==
      NULL) {
    cJSON_AddItemToObject(pJsonChild, secName, section = cJSON_CreateObject());
  }
  if (sRegData->bInvalid) {
    cd_snprintf_s(jsonItemString, PM_JSON_STRING_LEN, PM_FIXED_DATA_CC_RC, cc,
                  ret);
  ...
  cJSON_ArrayForEach(itRegs, regList) {
    ...
    if (reg.coreScope) {
      ...
      for (uint32_t u32CoreNum = 0; (cpuInfo->coreMask >> u32CoreNum) != 0;
        ...
        powerManagementJsonPerCoreICX1(jsonNameString, u32CoreNum, &sRegData,
                                       pJsonChild, cc, ret);
      }
    } else {
      ret =
      ...
      cd_snprintf_s(jsonNameString, PM_JSON_STRING_LEN, reg.regName);
      powerManagementJsonICX1(jsonNameString, &sRegData, pJsonChild, cc, ret);
    }
  }
  return ACD_SUCCESS;
}
...
int logPowerManagement(CPUInfo *cpuInfo, cJSON *pJsonChild) {
  ...
  for (uint32_t i = 0;
    ...
    if (cpuInfo->model == sPowerManagementVx[i].cpuModel) {
      return sPowerManagementVx[i].logPowerManagementVx(cpuInfo, pJsonChild);
    }
  }
