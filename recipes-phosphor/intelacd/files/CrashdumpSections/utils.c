...
bool isBigCoreRegVersionMatch(cJSON *root, uint32_t version) {
  ...
  if (child != NULL && enable) {
    ...
    if (decodeObject != NULL) {
      cJSON *versionObject = cJSON_GetObjectItem(decodeObject, jsonItemName);
  ...
  if (child != NULL && enable) {
    ...
    if (decodeObject != NULL) {
      ...
      if (versionObject != NULL) {
        return cJSON_GetObjectItemCaseSensitive(versionObject, "reg_list");
      }
...
cJSON *selectAndReadInputFile(Model cpuModel, char **filename,
  ...
  case cd_spr:
    ...
    break;
  default:
    CRASHDUMP_PRINT(ERR, stderr, "Error selecting input file (CPUID 0x%x).\n",
                    cpuModel);
    return NULL;
  }
...
calculateDelay(struct timespec *crashdumpStart,
  ...
  if (runTimeInNs < delayTimeFromInputFileInNs) {
    ...
    delay.tv_nsec = (delayTimeFromInputFileInNs - runTimeInNs) % (uint64_t)1e9;
