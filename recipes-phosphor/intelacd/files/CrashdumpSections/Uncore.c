...
static void uncoreStatusJsonICXSPR(const char *regName,
  ...
  if (sRegData->bInvalid) {
    cd_snprintf_s(jsonItemString, US_JSON_STRING_LEN, UNCORE_FIXED_DATA_CC_RC,
                  cc, ret);
  ...
  cJSON_ArrayForEach(itRegs, regList) {
    ...
    case US_REG_DWORD:
      ...
      ret = peci_RdEndPointConfigPciLocal_seq(
          cpuInfo->clientAddr, US_PCI_SEG, bus, pciReg.u8Dev, pciReg.u8Func,
          pciReg.u16Reg, pciReg.u8Size, (uint8_t *)&sRegData.uValue.u64,
          peci_fd, &cc);
      if (ret != PECI_CC_SUCCESS) {
    ...
    case US_REG_QWORD:
      for (uint8_t u8Dword = 0; u8Dword < 2; u8Dword++) {
        ret = peci_RdEndPointConfigPciLocal_seq(
            cpuInfo->clientAddr, US_PCI_SEG, bus, pciReg.u8Dev, pciReg.u8Func,
            pciReg.u16Reg + (u8Dword * 4), sizeof(uint32_t),
  ...
  cJSON_ArrayForEach(itRegs, regList) {
    ...
    cJSON_ArrayForEach(itParams, itRegs) {
      ...
      case US_MMIO_FUNCTION:
        ...
        break;
      case US_MMIO_OFFSET:
        mmioReg.u64Offset = strtoull(itParams->valuestring, NULL, 16);
        break;
...
int logUncoreStatus(CPUInfo *cpuInfo, cJSON *pJsonChild) {
  ...
  for (uint32_t i = 0;
    ...
    if (cpuInfo->model == sUncoreStatusLogVx[i].cpuModel) {
      revision_uncore =
          getCrashDataSectionVersion(cpuInfo->inputFile.bufferPtr, "uncore");
      logCrashdumpVersion(pJsonChild, cpuInfo, RECORD_TYPE_UNCORESTATUSLOG);
