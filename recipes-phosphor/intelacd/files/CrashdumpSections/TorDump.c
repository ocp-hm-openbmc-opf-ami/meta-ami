...
static void torDumpJsonICXSPR(uint32_t u32Cha, uint32_t u32TorIndex,
  ...
  if ((channel = cJSON_GetObjectItemCaseSensitive(pJsonChild, jsonItemName)) ==
      NULL) {
  ...
  cd_snprintf_s(jsonItemName, TD_JSON_STRING_LEN, TD_JSON_SUBINDEX_NAME,
                u32TorSubIndex);
  if (bInvalid) {
    cd_snprintf_s(jsonItemString, TD_JSON_STRING_LEN, TD_FIXED_DATA_CC_RC, cc,
                  ret);
    ...
    for (int i = u32PayloadBytes - 1; i >= 0; i--) {
      ...
      ptr +=
          cd_snprintf_s(ptr, u32PayloadBytes, "%02x", pu8TorCrashdumpData[i]);
    }
    if (PECI_CC_UA(cc)) {
      cd_snprintf_s(jsonErrorString, TD_JSON_STRING_LEN, TD_DATA_CC_RC, cc,
                    ret);
...
int logTorDumpICXSPR(CPUInfo *cpuInfo, cJSON *pJsonChild) {
  ...
  // Crashdump Enabled
  ret =
      peci_CrashDump_Discovery(cpuInfo->clientAddr, PECI_CRASHDUMP_ENABLED, 0,
                               0, 0, sizeof(uint8_t), &u8CrashdumpEnabled, &cc);
  ...
  // Agent Payload Size
  ret = peci_CrashDump_Discovery(cpuInfo->clientAddr, PECI_CRASHDUMP_AGENT_DATA,
  ...
  for (size_t cha = 0; cha < cpuInfo->chaCount; cha++) {
    for (uint32_t u32TorIndex = 0; u32TorIndex < TD_TORS_PER_CHA_ICX1;
      ...
      for (uint32_t u32TorSubIndex = 0;
        ...
        ret =
            peci_CrashDump_GetFrame(cpuInfo->clientAddr, PECI_CRASHDUMP_TOR,
                                    cha, (u32TorIndex | (u32TorSubIndex << 8)),
                                    u32PayloadBytes, pu8TorCrashdumpData, &cc);
        ...
        torDumpJsonICXSPR(cha, u32TorIndex, u32TorSubIndex, u32PayloadBytes,
                          pu8TorCrashdumpData, pJsonChild, bInvalid, cc, ret);
