...
int fillCPUID(CPUInfo *cpuInfo, char *cSectionName, cJSON *pJsonChild) {
  ...
  cd_snprintf_s(jsonItemName, SI_JSON_STRING_LEN, SI_JSON_SOCKET_NAME, cpuNum);
  if ((cpu = cJSON_GetObjectItemCaseSensitive(pJsonChild, jsonItemName)) ==
      NULL) {
    cJSON_AddItemToObject(pJsonChild, jsonItemName, cpu = cJSON_CreateObject());
  }
 ...
 *
 ******************************************************************************/
int fillCoreMaskSource(CPUInfo *cpuInfo, char *cSectionName,
                       cJSON *pJsonChild) {
  return fillSource(cpuInfo, cSectionName, pJsonChild,
 ...
 *
 ******************************************************************************/
int fillChaCountSource(CPUInfo *cpuInfo, char *cSectionName,
                       cJSON *pJsonChild) {
  return fillSource(cpuInfo, cSectionName, pJsonChild,
...
int fillUCodeVersion(CPUInfo *cpuInfo, char *cSectionName, cJSON *pJsonChild) {
  ...
  if (ret != PECI_CC_SUCCESS) {
    cd_snprintf_s(jsonItemString, SI_JSON_STRING_LEN, MD_FIXED_DATA_CC_RC, cc,
                  ret);
...
static int sysInfoJson(CPUInfo *cpuInfo, cJSON *pJsonChild) {
  ...
  for (i = 0; i < (sizeof(sSysInfoTable) / sizeof(SSysInfoSection)); i++) {
    r = sSysInfoTable[i].FillSysInfoJson(cpuInfo, sSysInfoTable[i].sectionName,
                                         pJsonChild);
    if (r == 1) {
      ret = ACD_FAILURE;
    }
...
int logSysInfoInputfile(CPUInfo *cpuInfo, cJSON *pJsonChild,
  ...
  for (i = 0;
       i < (sizeof(sSysInfoInputFileTable) / sizeof(SSysInfoInputFileSection));
       i++) {
...
int logResetDetected(cJSON *metadata, int cpuNum, int sectionName) {
  ...
  if (sectionName != DEFAULT_VALUE) {
    cd_snprintf_s(resetSection, SI_JSON_STRING_LEN, RESET_DETECTED_NAME, cpuNum,
                  sectionNames[sectionName].name);
