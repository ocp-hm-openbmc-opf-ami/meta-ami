  ...
  for (const std::pair<
    ...
    if (softwareVerIt != pathPair.second.end()) {
      ...
      if (purposeIt != softwareVerIt->second.end()) {
        ...
        if (bmcPurpose->compare("xyz.openbmc_project.Software.Version.BMC")) {
          boost::container::flat_map<std::string,
                                     std::variant<std::string>>::const_iterator
          ...
          if (versionIt != softwareVerIt->second.end()) {
            ...
            if (bmcVersion != nullptr) {
              size_t copySize = std::min(bmcVersion->size(), bmcVerStrSize - 1);
...
static int sysInfoCommonJson(cJSON *pJsonChild) {
  ...
  for (i = 0; i < (sizeof(sSysInfoCommonTable) / sizeof(SSysInfoCommonSection));
       i++) {
