...
static const std::vector<std::string> privilege_list = {
    priv_admin, priv_operator, priv_user};
                    ...
                    std::string replaceString, bool readWrite)
{
    ...
    while (std::getline(file, lineBuf))
    {
            ...
            (strstr(lineBuf.c_str(), matchString[i].c_str()) != nullptr))
        {
            if (readWrite == 1)
            {
                sscanf(lineBuf.c_str(), "%s%s", tempBuf.c_str(),
                       readConf.c_str());
...
void updateJsonField(const std::string& filename, const std::string& section,
                     const std::string& key, auto value)
{
    ...
    {
        std::cerr << "Failed to write updated JSON to file." << std::endl;
    }
}

int CreateDefaultJson(std::string filename)
{
...
int main()
{
    ...
    if (pid == 0)
    {
        ...
        std::ifstream file(RADIUSJSONCONF);
        if (!file.is_open())
        {
            if (CreateDefaultJson(RADIUSJSONCONF))
            {
                std::cerr << "Failed to create Radius JSON file." << std::endl;
                return 1;
            }
            file.open(RADIUSJSONCONF);
            if (!file.is_open())
            {
                std::cerr << "Failed to open newly created JSON file."
                          << std::endl;
                return 1;
            }
        }
        ...
        // Extract RadiusConfig
        std::string ip = data["RadiusConfig"].value("IP", "");
        int portnum = data["RadiusConfig"].value("PortNumber", 0);
        std::string password = data["RadiusConfig"].value("Password", "");
        bool enable = data["RadiusConfig"].value("Enable", false);
        bool eaptlsenable = data["RadiusConfig"].value("EnableEapTLS", false);

        // Extract role_map
        std::string group1 = data["role_map"].value("GroupName1", "");
        std::string group2 = data["role_map"].value("GroupName2", "");
        std::string group3 = data["role_map"].value("GroupName3", "");
        std::string priv1 = data["role_map"].value("Privilege1", "");
        std::string priv2 = data["role_map"].value("Privilege2", "");
        std::string priv3 = data["role_map"].value("Privilege3", "");

        radRoleIface->register_property(
            "GroupName1", group1,
            [&group1, &oldstring,
             &newstring](const std::string& requested, std::string& resp) {
                updateJsonField(RADIUSJSONCONF, "role_map", "GroupName1",
                                requested);
                resp = requested;
                return 1;
            });
        radRoleIface->register_property(
            ...
            [&priv1, &oldstring,
             &newstring](const std::string& requested, std::string& resp) {
                if (std::find(privilege_list.begin(), privilege_list.end(),
                              requested) != privilege_list.end())
                {
                    updateJsonField(RADIUSJSONCONF, "role_map", "Privilege1",
                                    requested);
                    resp = requested;
                    return 1;
                }
        ...
        radRoleIface->register_property(
            ...
            [&group2, &oldstring,
             &newstring](const std::string& requested, std::string& resp) {
                updateJsonField(RADIUSJSONCONF, "role_map", "GroupName2",
                                requested);
                resp = requested;
                return 1;
            });
            ...
            [&priv2, &oldstring,
             &newstring](const std::string& requested, std::string& resp) {
                if (std::find(privilege_list.begin(), privilege_list.end(),
                              requested) != privilege_list.end())
                {
                    updateJsonField(RADIUSJSONCONF, "role_map", "Privilege2",
                                    requested);
                    resp = requested;
                    return 1;
                }
        ...
        radRoleIface->register_property(
            ...
            [&group3, &oldstring,
             &newstring](const std::string& requested, std::string& resp) {
                updateJsonField(RADIUSJSONCONF, "role_map", "GroupName3",
                                requested);
                resp = requested;
                return 1;
            });
            ...
            [&priv3, &oldstring,
             &newstring](const std::string& requested, std::string& resp) {
                if (std::find(privilege_list.begin(), privilege_list.end(),
                              requested) != privilege_list.end())
                {
                    updateJsonField(RADIUSJSONCONF, "role_map", "Privilege3",
                                    requested);
                    resp = requested;
                    return 1;
                }
        ...
        radIface->register_property(
            "Enable", enable, [](const bool& requested, bool& resp) {
                updateJsonField(RADIUSJSONCONF, "RadiusConfig", "Enable",
                                requested);
                resp = requested;
                return 1;
            });
        ...
        radIface->register_property(
            "PortNumber", portnum, [](const int& requested, int& resp) {
                if (isPortInUse(requested) == true)
                {
                    updateJsonField(RADIUSJSONCONF, "RadiusConfig",
                                    "PortNumber", requested);
                    resp = requested;
                    return 1;
                }
                else
        ...
        radIface->register_property(
            "EnableEapTLS", eaptlsenable,
            [](const bool& requested, bool& resp) {
                updateJsonField(RADIUSJSONCONF, "RadiusConfig", "EnableEapTLS",
                                requested);
                resp = requested;
                return 1;
            });

        radIface->register_method("Send",
    ...
    else
    {
        ...
        {
            memset(&req, 0, sizeof(pamusrpkt_t));
            if (0 != get_pam_userinfo(&req, PAM_HELPER_Q, handle,
                                      WAIT_INFINITE_TIME))
            {
                printf(" %s : error fetching messages from queue %s \n",
            ...
            switch (req.action)
            {
                case ADD_PAM_USER:
                {
                    ...
                    if (user_found == 0)
                    {
                        pamusr_t* u = &pamusrtbl[req.pt][tblinfo[req.pt].index];
                        printf("Add Index: %d\n", tblinfo[req.pt].index);
                        ret = snprintf(u->name, sizeof(u->name), "%s",
                                       req.usr.name);
                        if (ret < 0 || ret >= (signed)sizeof(u->name))
                        {
                            printf("Buffer Overflow\n");
                        ...
                        if ((tblinfo[req.pt].uid - 1) ==
                            tblinfo[req.pt].last_uid)
                        {
                            tblinfo[req.pt].uid = tblinfo[req.pt].first_uid;
                        }
                        if (tblinfo[req.pt].index == NUM_OF_PAM_USERS)
                    ...
                    /* This function is used to call whenever user added in any
                     * table.*
                     * * when adding user, check whether any other database has
                     * the same name..
                     * * if then delete that user...*/
                    remove_pam_user(req.usr.name, req.pt);
                }
                ...
                case GET_USERINFO_BY_NAME:
                {
                    ...
                    {
                        u = (ut + j);
                        if (strncmp(u->name, req.usr.name,
                                    strlen(u->name) + 1) == 0)
                        {
                            memcpy((char*)&res.usr, (char*)u, sizeof(pamusr_t));
                            res.ret = PAM_USER_RETRIEVED_SUCCESSFULLY;
