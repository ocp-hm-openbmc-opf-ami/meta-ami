...
#define RADIUS_FIRST_UID 9000
#define RADIUS_LAST_UID 9999
...
#define NSS_RADIUS_Q "/var/NSSRADIUSQ"
...
#define PAMH_OPEN_Q(QUEUE, handle) handle = pipe_open(QUEUE, O_RDWR)
...
#define PAMH_GET_FROM_Q(buf, size, handle, perr)                               \
  {                                                                            \
    ...
    left = size;                                                               \
    while (left > 0) {                                                         \
      len = pipe_read(handle, (unsigned char *)buf + readlen, left);           \
      if (len < 0) {                                                           \
        if (errno == EAGAIN) {                                                 \
          continue;                                                            \
        } else {                                                               \
...
#define PAMH_ADD_TO_Q(buf, size, handle, perr)                                 \
  *perr = pipe_write(handle, buf, size)
