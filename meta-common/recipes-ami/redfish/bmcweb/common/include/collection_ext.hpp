    ...
    const dbus::utility::MapperGetSubTreePathsResponse& objects)
{
    ...
    if (check)
    {
        ...
        {
            redfish::getRaidDevices(asyncResp, systemName, jsonCountKeyName,
                                    jsonKeyName);
            redfish::getHBADevices(asyncResp, systemName, jsonCountKeyName,
                                   jsonKeyName);
        }
#endif
#ifdef ONETREE_BRCMRAID8
        {
            redfish::getSl8Devices(asyncResp, systemName, jsonCountKeyName,
                                   jsonKeyName);
        }
#endif
#ifdef ONETREE_MSCCRAID
        redfish::getMSCCDevices(asyncResp, systemName, jsonCountKeyName,
                                jsonKeyName);
#endif
...
inline void getCollectionToKey(
    ...
    const std::string& systemName, bool check = false)
{
    ...
    const std::string& systemName, bool check = false)
{
    getCollectionToKey(asyncResp, collectionPath, interfaces, subtree,
                       nlohmann::json::json_pointer("/Members"), systemName,
                       check);
}
