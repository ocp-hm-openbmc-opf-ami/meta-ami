...
{
    lg2::info(
        "[pldm-oem-hooks : oemExtendPlatformEvents] PLDM_OEM_EVENT_CLASS_0xFE={CMD}",
        "CMD", PLDM_OEM_EVENT_CLASS_0xFE);

    eventMap[PLDM_OEM_EVENT_CLASS_0xFE].emplace_back([](const pldm_msg* request,
                                                        ...
                                                        uint8_t&
                                                            platformEventStatus) {
        lg2::info(
            "[pldm-oem-hooks : PLDM_OEM_EVENT_CLASS_0xFE] payloadLength: {PL}, formatVersion: {FV}, tid: {TID}, eventDataOffset: {EDO}, platformEventStatus: {PES}",
            "PL", payloadLength, "FV", formatVersion, "TID", tid, "EDO",
            eventDataOffset, "PES", platformEventStatus);
        return 0;
    });
