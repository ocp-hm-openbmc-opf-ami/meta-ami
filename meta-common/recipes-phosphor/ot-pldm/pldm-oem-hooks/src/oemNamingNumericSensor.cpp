...
const std::map<std::string, std::string> customSensorNameTable = {
...
extern "C"
{
/** @brief Customize numeric sensor names for PLDM sensors
 *
 *  @param[out] sensorNameTable - The map of UUID to customized sensor names
 *  @return 0 on success, non-zero on failure
 */
int oemNamingNumericSensor(std::map<std::string, std::string>& sensorNameTable)
{
    lg2::info("[pldm-oem-hook] oemNamingNumericSensor called");
    sensorNameTable.insert(customSensorNameTable.begin(),
                           customSensorNameTable.end());
    return 0;
}
}
